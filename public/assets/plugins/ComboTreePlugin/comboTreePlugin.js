...
; (function ($, window, document, undefined) {
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
        //taha add && sourceItem.subs.length > 0
        var itemHtml = "",
            isThereSubs = sourceItem.hasOwnProperty("subs") && sourceItem.subs.length > 0;
    ...
    ComboTree.prototype.toggleSelectionTree = function (item, direction) {
        ...
        if (direction === undefined) {
            if ($(subMenu).is(':visible'))
                ...
                //taha add &minus; between span
                $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-down-circle-outline">&minus;</span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
    ...
    ComboTree.prototype.isItemInArray = function (item, arr) {
        for (var i = 0; i < arr.length; i++) {
            ...
            if (arr[i].hasOwnProperty("subs") && arr[i].subs.length > 0) {
    ...
    ComboTree.prototype.processSelected = function () {
        ...
        if (this.elemInput.value == '' || this.elemInput.value == null)
            this.refreshInputVal();
    };
    ...
    //taha add .toLowerCase()
    ComboTree.prototype.findItembyId = function (itemId, source) {
        if (itemId && source) {
            for (let i = 0; i < source.length; i++) {
                if (source[i].id.toLowerCase() == itemId.toLowerCase())
                    return { id: source[i].id, title: source[i].title };
                //taha add && source[i].subs.length > 0
                if (source[i].hasOwnProperty("subs") && source[i].subs.length > 0) {
