...
const $ = window.$;
export default function RadioButtonListHtml(props) {
                    ...
                    <div id={model.Id} className={model.CssClass} data-type={model.Type} data-parameter={model.Parameter}
                        ...
                        {...attr}>
                        {
                            ...
                            model.Options.map((item, index) => {
                                return <label key={index} className="radio radio-outline"  >
                                    <input defaultChecked={item.Selected} name={model.Id} type="radio" disabled={model.IsReadonly} defaultValue={item.Value} />
                                    <span></span>
                                    {item.Label}
                                </label>
                            })
